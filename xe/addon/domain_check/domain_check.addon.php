...
"
        ...
        if(!isset($addon_info->except_domain)){
            if ($_SERVER[HTTP_HOST]!=$addon_info->my_domain) Context::addHtmlHeader("<script type=\"text/javascript\"> self.location.href=\"http://".$addon_info->my_domain.$_SERVER['REQUEST_URI']."\"</script>");
        }else{
            ...
            foreach($except_domain as $value) {
                ...
                if($_SERVER[HTTP_HOST]!=$addon_info->my_domain){
                    Context::addHtmlHeader("<script type=\"text/javascript\"> self.location.href=\"http://".$addon_info->my_domain.$_SERVER['REQUEST_URI']."\"</script>");
                }
            }
        }
    }
